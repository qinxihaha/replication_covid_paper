---
title: "EMD 553 Final Project_Figures and Tables"
output:
  html_document:
    toc: yes
    toc_depth: 2
...
library(dampack)
library(plotly)
options (digits=2,scipen=200) 
```

# Inclusion of False Negative Compartment
```{r}
...
## Table 2 & Table 3  
```{r, warning=FALSE}
simu0(test_freq=2, test_se=0.7, R=2.5)  #original model, base-case scenario
simu(test_freq=2, test_se=0.7, beta_calc=0, R=2.5)  #new model, base-case scenario
```
...
# Optimization of Transmission Parameter 
```{r}
...
## Table 4  
```{r, warning=FALSE}
simu(test_freq=2, test_se=0.7, beta_calc=1)  #Connecticut-specified scenario
```

## Figure 3  
```{r, warning=FALSE}
iso_plot(simu(test_freq=1, test_se=0.7, beta_calc=1))  #daily testing  
iso_plot(simu(test_freq=2, test_se=0.7, beta_calc=1))  #testing per 2 days
iso_plot(simu(test_freq=7, test_se=0.7, beta_calc=1))  #Weekly testing  
iso_plot(simu(test_freq=99999999999999, test_se=0.7, beta_calc=1))  #symptom-based testing
```
...
```{r, warning=FALSE}
cea_output(beta_calc=0, R=2.5) #base-case scenario
cea_output(beta_calc=1)  #Connecticut-specified scenario
cea_output(beta_calc=0, R=3.5) #worst-case scenario
cea_output(beta_calc=0, R=1.5) #best-case scenario
```
